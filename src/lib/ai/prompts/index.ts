...
export function buildWebsiteScanPrompt(pages: { url: string; title: string; description?: string; text: string }[], socialLinks: { platform: string; url: string; source_url: string }[], siteType: string = "business"): PromptTemplate {
  const system = [
    ...
    "Degree programs and courses: list EACH program separately with its full name (e.g. 'B.Tech Computer Science', not generic 'B.Tech').",
...
export function buildWebsiteFactsPrompt(pages: CorpusPage[], siteType: string = "business"): PromptTemplate {
  const system = [
    ...
    "content_theme facts must be REAL value topics (problems, use-cases, outcomes the site talks about) — NEVER bare navigation labels like 'Services', 'Pricing', 'About', 'Contact', 'Home'. If a page only contains nav boilerplate, skip it instead of recording a theme.",
...
export function buildWebsiteSynthesisPrompt(
  ...
  const system = [
    ...
    "Degree programs and courses: list EACH program separately with its full name (e.g. 'B.Tech Computer Science', 'B.Tech Mechanical', 'MBA', 'BCA') — never merge branches into a generic 'B.Tech'/'MBA' entry.",
    "target_customers: prefer explicit audience segments from the site. If none are stated, INFER 2-4 plausible segments from the services + industries (e.g. a web agency serves small businesses, startups, local retailers) with confidence <= 0.5 and evidence 'inferred from services'. Never leave it empty when services exist.",
    ...
    "Separately, ALWAYS fill suggested_competitors (max 5) from your own market knowledge of this industry: well-known direct competitors with name, website_url and one-line reason, evidence_type ALWAYS 'industry_inference', confidence 0.25-0.45. Mark clearly that these are suggestions, not site evidence. Empty only if you genuinely know no competitors in this space.",
    "Separately, ALWAYS fill suggested_competitors (max 5) from your own market knowledge of this industry: well-known direct competitors with name, website_url and one-line reason, estimated ALWAYS true. Mark clearly that these are suggestions, not site evidence. Empty only if you genuinely know no competitors in this space.",
    "content_themes must be real value topics with descriptions — never bare nav labels ('Services', 'Pricing', 'About', 'Contact'). Drop any theme that is just a page name.",
  ...
  const user = [
    ...
    JSON.stringify({
      ...
      tone: "string or null",
      competitors: [{ name: "string", website_url: "string or null", reason: "string or null", source_url: "string", evidence_quote: "string or null", evidence_type: "comparison_page|mentioned_on_website|outbound_link|industry_inference", confidence: 0.8 }],
      suggested_competitors: [{ name: "string", website_url: "string or null", reason: "string or null", source_url: null, evidence_quote: null, evidence_type: "industry_inference", confidence: 0.35 }],
      confidence: 0.8,
    }),
  ].join("\n\n");
...
export function buildWhatsAppReplyPrompt(ctx: TaskContext): PromptTemplate {
  ...
  const system = [
    ...
    "Language: mirror them exactly. English in → English out. Devanagari Hindi → Hindi. Hinglish → Hinglish. Marathi (Devanagari script) → Marathi. Never start in Hinglish unless they did.",
