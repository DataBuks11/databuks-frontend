...
export interface ProcessWhatsAppResult {
   ...
   *  fast-path template or an LLM-failure fallback). Lets callers — e.g.
   *  the retry cron — distinguish a real AI reply from a fallback resend. */
  aiGenerated?: boolean;
  decision?: {
    allowed: boolean;
    ruleId?: string;
...
export async function retryQueuedAiReplies(
  ...
  for (const q of (((queued ?? []) as any[]).slice(0, limit))) {
    ...
    try {
      ...
      // Only a real LLM reply counts as "resent" — a fallback resend means
