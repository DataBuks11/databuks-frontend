...
export interface WhatsAppInboundMessage {
  ...
  mediaUrl?: string;
  /** Media kind â€” "image" | "video" | "audio" | "document" | "sticker" */
  mediaType?: string;
}
...
export interface ProcessWhatsAppResult {
  ...
  /** True only when the sent reply was generated by the LLM (not a
   *  fast-path template or an LLM-failure fallback). Lets callers â€” e.g.
   *  the retry cron â€” distinguish a real AI reply from a fallback resend. */
  aiGenerated?: boolean;
  decision?: {
    allowed: boolean;
    ruleId?: string;
...
export function normalizeWhatsAppPhone(remoteJid: string): string | null {
  ...
  // Internal WhatsApp IDs â€” store raw digits
  return cleaned;
}
...
async function findOrCreateLead(supabase: SupabaseClient, userId: string, phone: string, pushName: string): Promise<Record<string, any>> {
  // Search with both formats â€” phone might be stored with or without + prefix
...
export async function processIncomingWhatsAppMessage(
  ...
  // Show "typingâ€¦" to the sender IMMEDIATELY â€” before any DB work â€” so the
  ...
  // â”€â”€â”€ Reminder detection â”€â”€â”€
  ...
  try {
    context = await buildWhatsAppReplyContext(supabase, {
      ...
      // 6 messages (not 25): GLM 5.3-free reliably chokes on big payloads â€”
  ...
  // never set this â€” they must NOT be queued for AI retry.
  let llmFailed = false;

  // â”€â”€â”€ Fast-path: pre-cached replies for trivial messages â”€â”€â”€
  ...
  if (trimmed.length === 0) {
    ...
    // Common short conversational acknowledgments in Hindi/Marathi/Hinglish â€”
  ...
  if (usedFastPath) {
    console.log(`[LIB:ai:whatsapp] fast-path used for "${trimmed.slice(0, 30)}" â€” skipping LLM`);
  }

  if (!usedFastPath && context) {
    try {
      // First attempt â€” provider handles network retries internally.
      ...
      // Schema-validation retry â€” the LLM often returns loose JSON, so we
  ...
  // â”€â”€â”€ Fallback reply when the LLM failed both attempts â”€â”€â”€
  ...
  //      no dashes, no quotes, no business name drop) â€” just a one-word
  ...
  if (!replyText) {
    ...
    console.warn(`[LIB:ai:whatsapp] LLM did not return a valid reply â€” using fallback for message ${input.messageId}`);
  }

  // â”€â”€â”€ Human takeover / double-send guard â”€â”€â”€
  ...
  // 30-minute blanket silence was too aggressive â€” it kept the AI from
  // continuing the conversation the owner wanted it to handle.
  const HUMAN_IDLE_MS = 90 * 1000;
  // Human takeover: if the owner messaged this lead within the last 90s
  // (before OR after this inbound - i.e. owner is actively chatting), the
  // AI stays quiet. Only after 90s of owner silence does the AI reply, so
  // it never talks over the owner mid-conversation but never leaves a lead
  // hanging either.
  const idleCutoff = new Date(Date.now() - HUMAN_IDLE_MS).toISOString();
  const { data: recentOwnerMsgs } = await supabase
    .from("whatsapp_messages")
    .select("id, timestamp")
    .eq("user_id", input.userId)
    .eq("remote_jid", input.remoteJid)
    .eq("from_me", true)
    .gte("timestamp", idleCutoff)
    .order("timestamp", { ascending: false })
    .limit(1);
  let humanTakeover = (recentOwnerMsgs?.length ?? 0) > 0;
  ...
  if (!sendReply || !replyText || humanTakeover) {
    ...
    return {
      ...
      replySent: false,
      replyText: null,
      decision: { allowed: true, reason: humanTakeover ? "human takeover active â€” AI stayed quiet" : "no reply generated" },
  ...
  // â”€â”€â”€ Queue for AI retry â”€â”€â”€
  ...
  return {
    ...
    replySent: !sendError,
    replyText,
    aiGenerated: !usedFastPath && !llmFailed,
    decision: {
      allowed: true,
...
/** Suffix marking synthetic retry runs â€” retry runs must never queue again. */
 ...
 * a WHATSAPP_REPLY_RETRIED marker is recorded for `resent` (real AI reply
 * delivered) and `skipped_moved_on` (chat moved on) so those never re-run,
 * while `still_failing` stays eligible until it ages out. A message is only
 * retried when it is still the latest inbound in its conversation.
 */
export async function retryQueuedAiReplies(
  ...
  for (const q of (((queued ?? []) as any[]).slice(0, limit))) {
    ...
    try {
      ...
      // this conversation â€” otherwise the chat moved on and a late reply to
      ...
      // Only a real LLM reply counts as "resent" â€” a fallback resend means
      // the model is still down, so keep it retryable (bounded by the 24h
      // age window, NOT by a RETRIED marker) for the next cron run.
      if (res.replySent && res.aiGenerated) {
        result.resent += 1;
        await markRetried("resent", { reply_text: String(res.replyText ?? "").slice(0, 200) });
      } else {
        result.failed += 1;
      }
    } catch (err: any) {
